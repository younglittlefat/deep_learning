...
ih = InputHelper()
ih.load_data(os.path.join(root_dir, "video_features"), os.path.join(root_dir, "label/test_label_all"))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        for nn in xrange(sum_no_of_batches):
            ...
            accuracy, sim = sess.run([siameseModel.accuracy, siameseModel.temp_sim], feed_dict)
            for i in range(len(ih.name_list)):
                print "%s\t%s\t%d" % (ih.name_list[i][0], ih.name_list[i][1], int(sim[i]))
